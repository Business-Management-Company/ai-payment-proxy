...
const endpoints = [
  {
    ...
    description: "Issues a single-use virtual Visa card scoped to your AI agent. The card is immediately active with a hard spending limit — your agent cannot overspend. Ideal for autonomous purchasing workflows where a human is not in the loop.",
  ...
  },
  {
    ...
    description: "Returns the full card number, CVV, and expiry for an AI agent to use at checkout. Only call this immediately before making a purchase — treat card credentials like secrets.",
    body: null,
    ...
    <div className="min-h-screen bg-[#0a0f1e] text-white">
      ...
      <nav className="sticky top-0 z-50 border-b border-gray-800 px-8 py-5 flex justify-between items-center bg-[#0a0f1e]/90 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-6">
          <a href="/docs"      className="text-white text-sm">Docs</a>
      ...
      <div className="max-w-7xl mx-auto px-8 py-12">
        ...
        {/* ── Quickstart — full width above the two columns ── */}
        <div className="bg-[#111827] border border-[#4ade80]/20 rounded-2xl p-8 mb-10">
          ...
          <p className="text-gray-400 mb-8">AI Payment Proxy issues single-use virtual Visa cards for autonomous AI agents. Your agent creates a card with a hard spending limit, reveals the card number, makes a purchase, and the card auto-cancels.</p>
          ...
          <pre className="bg-[#0a0f1e] rounded-xl p-4 text-sm font-mono text-gray-300 overflow-x-auto whitespace-pre">
        ...
        <div className="flex gap-8">
          ...
          <div className="w-56 shrink-0">
            ...
            <div className="space-y-1 mb-8">
              {endpoints.map((e, i) => (
                <button
                  ...
                  className={"w-full text-left px-3 py-2 rounded-lg text-sm transition flex items-center gap-2 " + (activeEndpoint === i ? "bg-[#4ade80]/10 text-white" : "text-gray-400 hover:text-white")}
                >
                  <span className={"text-xs font-mono px-1.5 py-0.5 rounded shrink-0 " + methodColors[e.method]}>{e.method}</span>
            ...
            <div className="p-4 bg-[#111827] border border-gray-800 rounded-xl">
              ...
              <code className="text-xs text-[#4ade80] block bg-[#0a0f1e] p-2 rounded break-all">Authorization: Bearer aipp_live_...</code>
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2">
                  {["curl", "python", "node"].map(l => (
                    <button
                      key={l}
                      onClick={() => setLang(l)}
                      className={"text-xs px-3 py-1 rounded-lg transition " + (lang === l ? "bg-[#4ade80] text-black font-semibold" : "text-gray-400 hover:text-white border border-gray-700")}
                    >{l}</button>
                  ))}
                  <button
                    onClick={copyCode}
                    className="text-xs px-3 py-1 rounded-lg border border-gray-700 text-gray-400 hover:text-white transition"
                  >{copied ? "Copied!" : "Copy"}</button>
                </div>
              </div>
              <pre className="bg-[#111827] border border-gray-800 rounded-xl p-4 text-sm text-gray-300 overflow-x-auto font-mono leading-relaxed whitespace-pre">
                {getCode()}
              </pre>
            </div>
            ...
            <div>
              ...
              <pre className="bg-[#111827] border border-gray-800 rounded-xl p-4 text-sm text-[#4ade80] overflow-x-auto font-mono leading-relaxed whitespace-pre">
                {ep.response}
              </pre>
            </div>
          </div>
