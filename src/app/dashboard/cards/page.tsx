    ...
    try {
      const res = await fetch("/api/v1/cards", {
        ...
        headers: {
          ...
          "Authorization": `Bearer ${customer?.api_key_prefix}`,
        },
        body: JSON.stringify({ label, limit_usd: Number(limit), merchant_category: category || undefined }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setCards(prev => [data.data, ...prev]);
      setShowForm(false);
      setLabel(""); setLimit(""); setCategory("");
    } catch (err: unknown) {
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-[#111827] border border-gray-800 rounded-xl p-6 mb-6">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <input type="text" placeholder="Merchant category (optional)" value={category} onChange={e => setCategory(e.target.value)} className="w-full bg-[#1a2235] border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#4ade80]" />
      ...
      <div className="bg-[#111827] border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {cards.length === 0 ? (
              <tr><td colSpan={5} className="text-gray-500 text-center px-6 py-12">No cards yet. Create your first one!</td></tr>
            ) : cards.map(card => (
              <tr key={card.id} className="border-b border-gray-800/50 hover:bg-[#1a2235]">
                ...
                <td className="px-6 py-4 text-gray-400 text-sm">{card.merchant_category || "—"}</td>
